...
while (True):
    ...
    post = ibm_db.fetch_assoc(res)
    while post != False:
        post_time = post['TIMESTAMP']
        if post['TWEET_TIMESTAMP'] != None:
            post_time = post['TWEET_TIMESTAMP']
        lastHourDateTime = datetime.now() - timedelta(hours = 1.05)

        # check if entry was within the last hour
        if (post_time > lastHourDateTime):
            route = (post['FROM'], post['TO'])
