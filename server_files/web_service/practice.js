    ...
    if (query.feed == "general") {
      ...
      for (var i in data) {
        ...
        if ((tweet.indexOf(street_from) > -1) || (tweet.indexOf(street_to) > -1)) {
          ...
          // if tweet hasn't already been associated with this route
          if (copies.length == 0) {
            // write tweet to feed table
            var sql_insert_tweet = "insert into gqxbnfjt.feed (user, from, to, comment, congestion, tweet_timestamp) values (?,?,?,?,?,?);";
            params = [post.USER, origin, destination, tweet, post.CONGESTION, post.LAST_UPDATE];
      ...
      resp.details = decap(conn.querySync(sql_select_live))[0];
