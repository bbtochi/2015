...
with open('nytraffic.csv', 'rU') as in_f:
    ...
    for row in in_csv:
        ...
        o = (row[2]+" and "+row[3]+", NY").lower()
        d = (row[2]+" and "+row[4]+", NY").lower()
        title = "BETWEEN "+o+" AND "+d

        # get count info
        left, lefts, heights, labels, colors = 0, [], [], [], []
        for i in range(24):
            lefts.append(left)
            heights.append(int(row[i+7]))
    ...
    ax = f.add_axes([0.1,0.1,0.8,.8], xlabel='time of day', ylabel='vehicles/hour', title=title)
